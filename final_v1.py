...
def navigate_to_page(driver, url, check_element_xpath, timeout=20):
    logging.info(f"Navigating to {url}...")
    driver.get(url)
    try:
        # Wait until the check element is located on the page to confirm navigation
        WebDriverWait(driver, timeout).until(EC.presence_of_element_located((By.XPATH, check_element_xpath)))
        logging.info(f"Successfully navigated to {url}")
    except TimeoutException:
        logging.error(f"Navigation to {url} failed within {timeout} seconds.")
        driver.quit()
        raise Exception(f"Navigation to {url} failed.")
